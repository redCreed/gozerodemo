...
func (c customProductOperationModel) OperationProducts(ctx context.Context, status int64) ([]*ProductOperation, error) {
	var operations []*ProductOperation
	err := c.QueryRowsNoCacheCtx(ctx, &operations, fmt.Sprintf("select %s from %s where status=?", productOperationRows, c.table), status)
	if err != nil {
		return nil, err
	}
